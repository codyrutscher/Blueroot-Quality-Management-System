...
export default function Labels() {
  ...
  const fetchLabels = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const uniqueCompanies = [...new Set(data.labels?.map((label: Label) => label.company) || [])]
  ...
  const filteredLabels = labels.filter(label => {
    const matchesCompany = !selectedCompany || label.company === selectedCompany
  ...
  const handleDownload = async (label: Label) => {
    try {
      const response = await fetch(`/api/labels/download?path=${encodeURIComponent(label.filePath)}`)
      if (response.ok) {
  ...
  if (loading) {
    return (
      <div className="p-8 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-4">Loading Labels...</h2>
      </div>
    )
  }
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredLabels.map((label) => (
          <div key={label.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-semibold text-gray-900 truncate" title={label.filename}>
                  {label.filename}
                </h3>
                <p className="text-xs text-blue-600 font-medium mt-1">{label.company}</p>
                {label.productSku && (
                  <p className="text-xs text-gray-500 mt-1">SKU: {label.productSku}</p>
                )}
              </div>
            </div>
